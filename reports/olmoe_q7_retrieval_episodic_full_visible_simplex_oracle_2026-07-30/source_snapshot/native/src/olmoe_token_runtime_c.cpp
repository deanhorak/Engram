...
int engram_olmoe_token_copy_last_regular_entry_trace_v1(
    const void* handle, float* entry_mass,
    const std::size_t entry_mass_count, float* entry_values,
    const std::size_t entry_value_count, std::uint8_t* valid_kind,
    const std::size_t valid_kind_count, std::uint64_t* positions,
    const std::size_t position_count, char* error,
    const std::size_t error_capacity) {
  try {
    const auto* runtime =
        static_cast<const engram::OLMoETokenRuntime*>(handle);
    if (runtime == nullptr || !runtime->has_regular_entry_trace() ||
        entry_mass == nullptr || entry_values == nullptr ||
        valid_kind == nullptr || positions == nullptr ||
        entry_mass_count != runtime->last_regular_entry_mass().size() ||
        entry_value_count !=
            runtime->last_regular_entry_values().size() ||
        valid_kind_count !=
            runtime->last_regular_entry_valid_kind().size() ||
        position_count !=
            runtime->last_regular_entry_positions().size()) {
      throw std::invalid_argument(
          "native OLMoE regular-entry trace storage is invalid");
    }
    std::copy(runtime->last_regular_entry_mass().begin(),
              runtime->last_regular_entry_mass().end(), entry_mass);
    std::copy(runtime->last_regular_entry_values().begin(),
              runtime->last_regular_entry_values().end(), entry_values);
    std::copy(runtime->last_regular_entry_valid_kind().begin(),
              runtime->last_regular_entry_valid_kind().end(), valid_kind);
    std::copy(runtime->last_regular_entry_positions().begin(),
              runtime->last_regular_entry_positions().end(), positions);
    return 0;
  } catch (const std::exception& exception) {
    error_text(error, error_capacity, exception.what());
    return 1;
