...
| 4. Shared recurrent controller and layer-free Engram runtime | Partial; state-transition gate passed | The standalone CPU controller runtime replays exact semantic/episodic streams with zero decoder-layer calls and terminal normalized MSE 0.0000208009 on the held-out trajectory. Stage-local causal K/V memory and a normalized-residual target are serialized and evaluated, but the best held-out learned-provider terminal MSE remains 0.1666128 (rank-256 nonlinear screen), versus the fixed 0.0225 promotion threshold; causal provider/controller promotion remains blocked. A separate FineWeb-Edu-derived auxiliary capture is prepared but the packaged teacher is currently killed by the host memory boundary before trace capture. |
...
| 6. Native C++ runtime, kernels, mapping, parity, generation, benchmarks | Partial/usable | CPU scalar/vector kernels, memory mapping, C ABI parity, native generation, and tests are operational. Python provider artifacts now reload read-only via NumPy memmap, reducing deployment RAM pressure without changing checksummed values. End-to-end long-context benchmarks and optimization remain. |
