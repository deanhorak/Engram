...
from engram.training.entropy_q3_codec import EntropyQ3LayerWeights
from engram.training.interleaved_entropy_q3_codec import (
...
def _save_fixture(path: Path, layer_count: int = 3):
    ...
    return load_interleaved_entropy_q3_artifact(path), hidden, source_intermediate


def test_interleaved_entropy_q3_is_deterministic_and_all_record_traffic_is_file(
    tmp_path,
):
    first = tmp_path / "first.interleaved-q3.bin"
    second = tmp_path / "second.interleaved-q3.bin"
    ...
    artifact = load_interleaved_entropy_q3_artifact(first)
    all_ids = [list(range(layer.width)) for layer in artifact.layers]
    traffic = interleaved_entropy_q3_dynamic_traffic(artifact, all_ids)
...
def test_interleaved_entropy_q3_selected_forward_matches_full_decode(tmp_path):
    artifact, _, _ = _save_fixture(tmp_path / "interleaved-q3.bin")
    decoded = decode_interleaved_entropy_q3_artifact(artifact)
    states = np.asarray(
    ...
    actual = interleaved_entropy_q3_forward(artifact, 0, states, selected)
...
def test_interleaved_entropy_q3_rejects_encoded_and_padding_corruption(tmp_path):
    path = tmp_path / "interleaved-q3.bin"
    ...
    with pytest.raises(ValueError, match="checksum"):
        load_interleaved_entropy_q3_artifact(corrupt_encoded)
    ...
    with pytest.raises(ValueError, match="padding"):
        load_interleaved_entropy_q3_artifact(corrupt_padding)


def test_interleaved_entropy_q3_router_bytes_are_cache_line_aligned(tmp_path):
    artifact, _, _ = _save_fixture(tmp_path / "interleaved-q3.bin")
    baseline = interleaved_entropy_q3_dynamic_traffic(artifact, 3)
    routed = interleaved_entropy_q3_dynamic_traffic(
