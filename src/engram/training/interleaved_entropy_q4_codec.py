"""Strict record-interleaved signed-Q4 entropy artifact.
...
_MAGIC = b"ENGEQ4I1"
_LAYER_MAGIC = b"ENGEQ4IL"
_RECORD_MAGIC = b"EQ4I"
_MODEL_MAGIC = b"EQ4M"
_VERSION = 1
_CODEC = 3
_SCALE_BITS = 12
_ALPHABET = 15
_MODEL_BLOCK_BYTES = 128
_RECORD_HEADER_BYTES = 64
_RECORD_HEADER = struct.Struct("<4sHHIIHH8I3H6x")
_MODEL_HEADER = struct.Struct("<4sHBBHHII")


@dataclass(frozen=True)
class EntropyQ4LayerWeights:
    gate: ArrayLike
    up: ArrayLike
    down: ArrayLike
...
def _quantize_rows(values: ArrayLike) -> _QuantizedRows:
    matrix = _matrix(values, "entropy-Q4 matrix")
    ...
    if np.any(~np.isfinite(scales)) or np.any(scales <= 0):
        raise ValueError("entropy-Q4 row scale cannot be represented as FP16")
    signed = np.clip(
        np.rint(matrix / scales.astype(np.float32)[:, None]), -7, 7
    ).astype(np.int8)
    symbols = (signed.astype(np.int16) + 7).astype(np.uint8)
...
def _decode_symbols(
    ...
    if values.ndim != 2 or values.dtype != np.uint8:
        raise ValueError("entropy-Q4 symbols must be a uint8 matrix")
    if np.any(values >= _ALPHABET):
        raise ValueError("entropy-Q4 stream contains a forbidden signed code")
    if row_scales.dtype != np.float16 or row_scales.shape != (len(values),):
        raise ValueError("entropy-Q4 scales must contain one FP16 value per row")
    if not np.all(np.isfinite(row_scales)) or np.any(row_scales <= 0):
        raise ValueError("entropy-Q4 scales must be finite and positive")
    signed = values.astype(np.int8) - np.int8(7)
...
def _model_block(
    ...
    if (
        len(flattened) != 3 * _ALPHABET
        or any(value <= 0 for value in flattened)
        or any(sum(table) != 1 << _SCALE_BITS for table in frequencies)
    ):
        raise ValueError("entropy-Q4 model has invalid frequency tables")
    ...
    block[_MODEL_HEADER.size : _MODEL_HEADER.size + len(raw)] = raw
    return bytes(block)
...
def _parse_model(
    ...
    if len(block) != _MODEL_BLOCK_BYTES:
        raise ValueError("entropy-Q4 model block size is invalid")
    (
        magic,
        version,
        scale_bits,
        ...
        frequency_bytes,
        checksum,
    ) = _MODEL_HEADER.unpack_from(block)
    expected_bytes = 3 * _ALPHABET * 2
    if (
        magic != _MODEL_MAGIC
        or version != _VERSION
        or scale_bits != _SCALE_BITS
        or alphabet != _ALPHABET
        or projection_count != 3
        or reserved
        or frequency_bytes != expected_bytes
    ):
        raise ValueError("entropy-Q4 model header is invalid")
    start = _MODEL_HEADER.size
    stop = start + frequency_bytes
    raw = block[start:stop]
    if _crc(raw) != checksum or any(block[stop:]):
        raise ValueError("entropy-Q4 model checksum/padding is invalid")
    values = tuple(int(value) for value in np.frombuffer(raw, dtype="<u2"))
    result = (
        values[:_ALPHABET],
        values[_ALPHABET : 2 * _ALPHABET],
        values[2 * _ALPHABET :],
    )
    if any(
        any(value <= 0 for value in table) or sum(table) != 1 << _SCALE_BITS
        for table in result
    ):
        raise ValueError("entropy-Q4 model frequencies are invalid")
    return result


@dataclass(frozen=True)
class EncodedInterleavedEntropyQ4Record:
    gate_encoded: bytes
    up_encoded: bytes
    down_encoded: bytes
...
@dataclass(frozen=True)
class LoadedInterleavedEntropyQ4Layer:
    frequencies: tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...]]
    records: tuple[EncodedInterleavedEntropyQ4Record, ...]
    hidden_size: int
    width: int
...
@dataclass(frozen=True)
class LoadedInterleavedEntropyQ4Artifact:
    layers: tuple[LoadedInterleavedEntropyQ4Layer, ...]
    hidden_size: int
    source_intermediate_size: int
...
def _interleaved_record_block(
    ...
    *,
    ...
    if any(values.shape != (columns,) for values in projections):
        raise ValueError("interleaved entropy-Q4 record rows must share one width")
    encoded_parts = tuple(
...
def _prepare_interleaved_layer(
    ...
    frequencies = tuple(
        normalized_frequencies(values.symbols.reshape(-1), alphabet_size=_ALPHABET)
...
def save_interleaved_entropy_q4_artifact(
    path: str | Path,
    layers: list[EntropyQ4LayerWeights] | tuple[EntropyQ4LayerWeights, ...],
    *,
    source_intermediate_size: int,
    ...
    if any(layer.hidden_size != hidden_size for layer in prepared):
        raise ValueError("all interleaved entropy-Q4 layers must share one hidden size")
    ...
    if destination.stat().st_size != cursor:
        raise AssertionError("serialized interleaved entropy-Q4 size is inconsistent")
    return destination
...
def _decode_record_payload(
    ...
    *,
    ...
    EncodedInterleavedEntropyQ4Record,
    tuple[NDArray[np.uint8], NDArray[np.uint8], NDArray[np.uint8]],
]:
    if len(block) < _RECORD_HEADER_BYTES:
        raise ValueError("interleaved entropy-Q4 record is shorter than its header")
    ...
    if (
        ...
        raise ValueError("interleaved entropy-Q4 record header is invalid")
    ...
    if any(not np.isfinite(value) or value <= 0 for value in scales):
        raise ValueError("interleaved entropy-Q4 record scale is invalid")
    encoded_stop = _RECORD_HEADER_BYTES + sum(lengths)
    if encoded_stop > len(block):
        raise ValueError("interleaved entropy-Q4 encoded lengths exceed the record")
    encoded = bytes(block[_RECORD_HEADER_BYTES:encoded_stop])
    if _crc(encoded) != encoded_crc:
        raise ValueError("interleaved entropy-Q4 encoded checksum is invalid")
    if (
        _crc(
            _record_integrity_payload(
                layer_index=layer_index,
        ...
        raise ValueError("interleaved entropy-Q4 record checksum is invalid")
    if any(block[encoded_stop:]):
        raise ValueError("interleaved entropy-Q4 record padding is non-zero")
    ...
    if any(
        ...
        raise ValueError(
            "interleaved entropy-Q4 decoded symbol checksum/code is invalid"
        )
    record = EncodedInterleavedEntropyQ4Record(
        encoded_parts[0],
        encoded_parts[1],
        encoded_parts[2],
        scales[0],
...
def load_interleaved_entropy_q4_artifact(
    path: str | Path,
) -> LoadedInterleavedEntropyQ4Artifact:
    ...
    try:
        ...
        raise ValueError(
            f"cannot read interleaved entropy-Q4 artifact {source}"
        ) from exc
    if len(payload) < _HEADER_BYTES:
        raise ValueError("interleaved entropy-Q4 artifact is shorter than its header")
    (
        magic,
        version,
        layer_count,
    ...
    if magic != _MAGIC or version != _VERSION:
        raise ValueError("interleaved entropy-Q4 artifact magic/version mismatch")
    for value, name in (
    ...
    if (
        ...
        raise ValueError("interleaved entropy-Q4 codec/alignment metadata is invalid")
    header_block_bytes = _align(_HEADER_BYTES, cache_line_bytes)
    ...
    if directory_block_bytes != expected_directory_block:
        raise ValueError("interleaved entropy-Q4 directory block size is invalid")
    ...
    if len(payload) < directory_end:
        raise ValueError(
            "interleaved entropy-Q4 artifact is shorter than its directory"
        )
    if any(payload[_HEADER.size : header_block_bytes]):
        raise ValueError("interleaved entropy-Q4 global-header padding is non-zero")
    ...
    if (
        ...
        raise ValueError(
            "interleaved entropy-Q4 global/directory integrity checksum is invalid"
        )
    if any(payload[directory_payload_end:directory_end]):
        raise ValueError("interleaved entropy-Q4 directory padding is non-zero")
    entries = tuple(
    ...
    for index, entry in enumerate(entries):
        ...
        if (
            ...
            raise ValueError("interleaved entropy-Q4 directory entry is invalid")
        if layer_offset + _LAYER_HEADER_BYTES > len(payload):
            raise ValueError("interleaved entropy-Q4 layer header is truncated")
        (
            layer_magic,
            layer_version,
            declared_index,
        ...
        if (
            ...
            raise ValueError("interleaved entropy-Q4 layer header is invalid")
        cursor = layer_offset + _LAYER_HEADER_BYTES
        model_stop = cursor + model_block_bytes
        if model_stop > len(payload):
            raise ValueError("interleaved entropy-Q4 model block is truncated")
        ...
        if offset_stop > len(payload):
            raise ValueError("interleaved entropy-Q4 offset block is truncated")
        raw_offsets = payload[cursor : cursor + offset_payload_bytes]
        if _crc(raw_offsets) != offset_crc:
            raise ValueError("interleaved entropy-Q4 offset checksum is invalid")
        if any(payload[cursor + offset_payload_bytes : offset_stop]):
            raise ValueError("interleaved entropy-Q4 offset padding is non-zero")
        ...
        if (
            ...
            raise ValueError("interleaved entropy-Q4 record offsets are invalid")
        records = []
        source_ids = []
        for record_index in range(width):
            ...
            if stop > len(payload) or (stop - start) % cache_line_bytes:
                raise ValueError("interleaved entropy-Q4 record block is invalid")
        ...
        if len(set(source_ids)) != width:
            raise ValueError("interleaved entropy-Q4 source record IDs are not unique")
        cursor = int(offsets[-1])
        if cursor != layer_offset + layer_block_bytes:
            raise ValueError("interleaved entropy-Q4 layer length is inconsistent")
        loaded_layers.append(
            LoadedInterleavedEntropyQ4Layer(
                frequencies,
                tuple(records),
                hidden_size,
                width,
    ...
    if expected_layer_offset != len(payload):
        raise ValueError(
            "interleaved entropy-Q4 artifact has trailing or missing bytes"
        )
    return LoadedInterleavedEntropyQ4Artifact(
        tuple(loaded_layers),
        hidden_size,
        source_intermediate_size,
        cache_line_bytes,
...
def _decode_interleaved_record(
    layer: LoadedInterleavedEntropyQ4Layer, record_index: int
) -> tuple[NDArray[np.float32], NDArray[np.float32], NDArray[np.float32]]:
    if record_index < 0 or record_index >= layer.width:
        raise ValueError("record_index is outside the interleaved entropy-Q4 layer")
...
def decode_interleaved_entropy_q4_artifact(
    artifact: LoadedInterleavedEntropyQ4Artifact,
...
def interleaved_entropy_q4_forward(
    artifact: LoadedInterleavedEntropyQ4Artifact,
    layer_index: int,
    hidden: ArrayLike,
    ...
    if (
        ...
        raise ValueError("layer_index is outside the interleaved entropy-Q4 artifact")
...
def interleaved_entropy_q4_dynamic_traffic(
    artifact: LoadedInterleavedEntropyQ4Artifact,
    ...
    *,
    ...
    return {
        "layout": "aligned_interleaved_gate_up_down_entropy_q4_records_v1",
