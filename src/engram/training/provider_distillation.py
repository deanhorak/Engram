...
def distill_stage_causal_attention_operator_provider(
    provider: str | Path,
    controller: str | Path,
    trace: str | Path,
    out: str | Path,
    *,
    ...
    """Distill stage-specific causal key/value memories through the controller."""
    ...
    def pack(data):
        ...
        counts = [int(np.sum(data.sample_id == sample)) for sample in sample_ids]
        if not counts or len(set(counts)) != 1:
            raise ValueError("stage causal traces must contain equal-length sequences")
        order = np.concatenate(
            [np.flatnonzero(data.sample_id == sample) for sample in sample_ids]
        )
        sequence = counts[0]
    ...
    key_projection = torch.nn.Parameter(
        torch.as_tensor(
            normal((stages, width, key_dim), 1.0 / np.sqrt(width)), device=device
        )
    )
    value_projection = torch.nn.Parameter(
        torch.as_tensor(
            normal((stages, width, value_dim), 1.0 / np.sqrt(width)), device=device
        )
    )
    state_query_projection = torch.nn.Parameter(
        torch.as_tensor(normal((width, query_width), 1.0 / np.sqrt(width)), device=device)
    ...
    correction_width = 2 * width if direct_hidden_correction else 2 * rank
    correction_head = torch.nn.Parameter(
        torch.zeros((stages, 2 * query_width + value_dim + 1, correction_width), device=device)
    )
    trainable = [
        key_projection,
        value_projection,
    ...
    def rollout(states, tokens, *, teacher_forcing: bool = False):
        ...
        stage_values: list[torch.Tensor] = [
            torch.empty((states.shape[0], 0, value_dim), dtype=torch.float32, device=device)
            for _ in range(stages)
        ]
        stage_losses = []
        terminal = []
        for position in range(states.shape[1]):
            ...
            for stage in range(stages):
                source = states[:, position, stage] if teacher_forcing else state
                current_key = source @ key_projection[stage]
                current_value = source @ value_projection[stage]
                history_keys = torch.cat(
                    (stage_keys[stage], current_key[:, None, :]), dim=1
                )
                history_values = torch.cat(
                    (stage_values[stage], current_value[:, None, :]), dim=1
                )
                features = torch.cat(
                    (source, token, torch.ones((states.shape[0], 1), device=device)), dim=-1
                )
                ...
                query_features = torch.cat(
                    (source @ state_query_projection, token @ token_query_projection), dim=-1
                )
                query = query_features @ query_head[stage]
                scores = torch.sum(history_keys * query[:, None, :], dim=-1) / np.sqrt(key_dim)
                weights = torch.softmax(scores, dim=-1)
                context = torch.sum(weights[..., None] * history_values, dim=1)
                correction_features = torch.cat(
                    (query_features, context, torch.ones((states.shape[0], 1), device=device)),
                    dim=-1,
                )
                correction = correction_features @ correction_head[stage]
                if direct_hidden_correction:
                    semantic = base_semantic + correction[:, :width]
                    episodic = base_episodic + correction[:, width:]
                else:
                    semantic = base_semantic + correction[:, :rank] @ tensors["semantic_basis"][stage]
                    episodic = base_episodic + correction[:, rank:] @ tensors["episodic_basis"][stage]
                state = controller_module.step(
                ...
                stage_keys[stage] = history_keys
                stage_values[stage] = history_values
                target = states[:, position, stage + 1]
                rms = target.square().mean(dim=-1, keepdim=True).sqrt().clamp_min(1e-4)
                stage_losses.append(((state - target) / rms).square().mean())
            terminal.append(state)
    ...
    trained = StageCausalAttentionOperatorStreamProvider(
        ...
        metadata_payload={
            ...
            "source_model_hash": training.manifest.get("model_hash"),
            "source_dataset_hash": training.manifest.get("dataset_hash"),
            "architecture": "stage_causal_key_value_context_over_pca_residual",
            "learned": True,
            "training_steps": steps,
            "teacher_forcing_steps": teacher_forcing_steps,
            "training_batch_size": batch_size,
